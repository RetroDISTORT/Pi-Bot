...
def start_app(device, fonts, appDirectory):
    try:
        ...
        printMessage(device, fonts[1], "App crashed")
...
def printMessage(device, inputFont, message):
    ...
    draw.rectangle((2, 2, device.width-2, device.height - 2), outline=0, fill=255)
    draw.text((4 , 32), message, font = inputFont, fill = 0)
    device.image(image)
    device.show()

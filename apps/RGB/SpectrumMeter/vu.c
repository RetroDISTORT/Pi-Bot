...
static int32_t         *audio_buffer_copy = NULL; /* audio_buffer_copy[audio_samples][audio_channels] */
...
static void *worker(void *unused)
{
	...
	audio_buffer_copy = malloc(sizeof(audio_buffer));
	memcpy(audio_buffer_copy, audio_buffer, sizeof(audio_buffer));
...
void vu_sample(int32_t *to){
  pthread_mutex_lock(&peak_lock);
  to = malloc(sizeof(audio_buffer_copy));
  memcpy(to, audio_buffer_copy, sizeof(audio_buffer_copy));
  pthread_mutex_unlock(&peak_lock);
}

...
class Menu:
    ...
    def drawMenu(self, options):
        self.draw.rectangle((0, 0, self.display.width, self.display.height), outline=0, fill=0)
        self.draw.rectangle((0, 26, self.display.width, 40), outline=0, fill=255)
        for i in range(-2,3):
            if len(options) > abs(i):
                self.draw.text((self.hshift[i] , self.vshift[i]), options[i], font=self.font,fill = 0 if i == 0 else 255)
